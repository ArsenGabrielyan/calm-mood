"use client"
import Image from "next/image";
import { Link } from "@/i18n/navigation";
import {Menu,X} from "lucide-react" // TODO: Make it responsive and implement sticky navbar
import { useTranslations } from "next-intl";

const links = [
     {name: "links.sounds", url: "/sounds"},
     {name: "links.exercise", url: "/breathing-exercise"},
]

export default function AppHeader(){
     const t = useTranslations("index")
     return (
          <header className="bg-[#2d5e0d] text-white py-2 px-4 flex justify-between items-center gap-2 fixed top-0 left-0 z-20 w-full">
               <Link href="/" title={t("backButton")}><Image src={t("logo.original")} alt="logo" width={250} height={75}/></Link>
               <ul className="flex gap-4 items-center justify-center">
                    {links.map((link,i)=>(
                         <li key={`link-${i}`} className="text-base md:text-lg pb-1 border-b-2 border-b-transparent hover:border-b-white capitalize"><Link href={link.url} title={t(link.name)}>{t(link.name)}</Link></li>
                    ))}
               </ul>
          </header>
     )
}